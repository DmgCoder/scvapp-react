...
export default function ScheduleItem(props) {
  ...
  async function getScheduleData() {
    let json = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/user/schedule`,
      {
        mode: "cors",
        credentials: "include",
        method: "GET",
    ...
    if (
      ...
      if (scheduleData.trenutnoNaUrniku.naslednjaUra.zacetekUreM >= 0) {
        let inter = setInterval(() => {
          ...
          let differenc =
            scheduleData.trenutnoNaUrniku.naslednjaUra.zacetekUreM - utcZdaj;
  ...
  if (scheduleData.trenutnoNaUrniku) {
    return (
      <div className="scheduleItem" style={props.style}>
        <p>Sedaj</p>
        <ClassInfo
          trenutnoNaUrniku={scheduleData.trenutnoNaUrniku}
          userData={props.userData}
        />
        <p>{`Sledi (Čez ${timeLeft}):`}</p>
        <ClassInfo
          trenutnoNaUrniku={scheduleData.trenutnoNaUrniku.naslednjaUra}
          userData={props.userData}
        />
      </div>
    );
...
function ClassInfo(props) {
  ...
  function changeIndexForUra(e) {
    ...
    if (targetName === "left") {
      ...
      //indexUp
      setIndexForUra(
        indexForUra + 1 < props.trenutnoNaUrniku.ura.length
          ? indexForUra + 1
          : indexForUra
      );
    }
  ...
  if (
    ...
    content = (
      <div
        ...
        onClick={showHideMoreInfo}
      >
        <p className="classInfo-krajsava">
          {props.trenutnoNaUrniku.ura[indexForUra]
            ? props.trenutnoNaUrniku.ura[indexForUra].krajsava || "/"
            : "/"}
        </p>
        <div className="classInfo-ucilnica-ikona">
          <p className="classInfo-ucilnica">
            {props.trenutnoNaUrniku.ura[indexForUra]
  ...
  return (
    <div className="classInfo" ref={classInfoRef}>
      <div className="classInfo-onelLine">
        <button
          name="left"
          className="classInfo-btn"
          onClick={changeIndexForUra}
          style={{ visibility: indexForUra > 0 ? "" : "hidden" }}
        >
          <svg
      ...
      {props.trenutnoNaUrniku.ura[indexForUra] && (
        <FloatingInfoWindow
          style={{
            display: showOtherInfo ? "" : "none",
            opacity: showOtherInfo ? "1" : "0",
            transition: "visibility 0.1s, opacity 0.2s ease-in-out",
            backgroundColor: props.userData.school
              ? props.userData.school.color
              : "",
          }}
          ura={props.trenutnoNaUrniku.ura[indexForUra]}
          trajanje={props.trenutnoNaUrniku.trajanje}
        />
      )}
      {showOtherInfo && (
        <div
          className="classInfo-floatingWindow-close"
          onClick={showHideMoreInfo}
        ></div>
      )}
    </div>
